...
const FormComponent = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <label>
        Phone:
        <input type="tel" required placeholder="12345" value={phone} onChange={(event) => setPhone(event.target.value)} />
      </label>
      <br />
      <label>
        Address:
        <textarea value={address} required placeholder="Create a message here..."
              cols="30"
              rows="10" onChange={(event) => setAddress(event.target.value)} />
      </label>
      <br />
